...
cutoff_freq_top=Fs/3;
cutoff_freq_bottom=0;
my_freqs=[];
if nargin>2,
    cutoff_freq_top=varargin{1};
end
if nargin>3,
    cutoff_freq_bottom=varargin{2};
end
if nargin>4,
    my_freqs=varargin{3};
end
...
y2=filterfft(f, y, cutoff_freq_top,cutoff_freq_bottom, my_freqs); % filter amplitudes
...
% plot(t*1e6,X)
% xlabel('uSec')
% if isempty(cutoff_freq_bottom),
%     scutoff='No cutoff.';
% else
%     scutoff=sprintf('Cutoff = %g [Mhz]', cutoff_freq_bottom/1e6);
% end
% stitle3=sprintf('Reconstructed signal with %d selected frequencies; %s', nf1, scutoff);
% title(stitle3)
% axis tight
return
function y2=filterfft(f, y, cutoff_top,cutoff_bottom, wins)
nf=length(f);
ny=length(y);
if ~(ny/2+1 == nf),
...
% cutoff filter
y2=zeros(1,ny);
if ~isempty(cutoff_top)
    ind1=find(f<=cutoff_top);
    ind2 = find(f(ind1)>=cutoff_bottom);
    y2(ind2) = y(ind2); % insert required elements
else
    y2=y;
end
