function [model] = chris_vol3d(varargin)
...
if(ind==3 || is3DTexture )    
  x = model.xdata;%[model.xdata(1), model.xdata(2); model.xdata(1), model.xdata(2)];
  y = model.ydata;%[model.ydata(1), model.ydata(1); model.ydata(2), model.ydata(2)];
  z = model.zdata;%[model.zdata(1), model.zdata(1); model.zdata(1), model.zdata(1)];
  for n = 1:size(cdata,3)
   cslice = squeeze(cdata(:,:,n,:));
   aslice = double(squeeze(alpha(:,:,n)));
   h(handle_ind) = surface(squeeze(x(:,:,n)),squeeze(y(:,:,n)),squeeze(z(:,:,n)),cslice,'alphadata',aslice,opts{:});
