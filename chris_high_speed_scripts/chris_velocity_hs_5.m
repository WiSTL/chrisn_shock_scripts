function [psi,ncount,err,a,b,gamma] = chris_velocity_hs_5(C,x,eta,tau,h,L)
...
%         p(:,:,k) = psi(:,:,k-1);
%     end
    [psi(:,:,k),ncount(k),err(k)] = forward_diff_sor_4((squeeze(a(:,:,k))),(squeeze(b(:,:,k))),p(:,:,k),(squeeze(gamma(:,:,k))),ne,nx,dx,de);
end
